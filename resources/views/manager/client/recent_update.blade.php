                  ...
                  <div id="leftContainer" class="col-lg-3">
                    <div class="row">
                      <div class="col-12 text-right my-3">
                        <a href="{{ route('allClientManager') }}" class="btn bg-theme text-light btn-outline-dark">
...
<script>
    $(document).ready(function () {
        ...
        $('#recentUpdate-saveButton').click(function (event) {
            ...
            $.ajax({
                url: "{{ route('recent-updates.store.manager') }}", 
                type: 'POST',
                data: formData,
                headers: {
        ...
        $('#recentUpdate-updateButton').click(function (event) {
            ...
            $.ajax({
                url: `{{ url('/manager/recent-updates') }}/${id}`,
                type: 'POST',
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $('#recentUpdateTable').on('click', '.delete-recent-update', function () {
            ...
            if (confirmDelete) {
                $.ajax({
                    url: `{{ url('/manager/recent-updates') }}/${update.id}`,
                    type: 'DELETE',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
